...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    console.log('Error while getting res from BE '+error);
...
export const progressAPI = {
  ...
  getStudentProgress: (studentId: number) =>api.get(`/progress/student/${studentId}`),
  enrollStudent: (studentId: number, curriculumId: number) =>api.post(`/progress/enroll/${studentId}`, { curriculumId }),
  startActivity: (activityId: number) =>api.post(`/progress/activity/${activityId}/start`),
  completeActivity: (activityId: number, earnedPoints: number, passed: boolean) =>api.post(`/progress/activity/${activityId}/complete`, { earnedPoints, passed }),
  submitQuiz: (quizId: number, correctAnswers: number, timeSpent: number) =>api.post(`/progress/quiz/${quizId}/submit`, { correctAnswers, timeSpent }),  
  getQuizAttempts: (quizId: number) =>api.get(`/progress/quiz/${quizId}/attempts`),
};

export const adminAPI = {
  getSystemStats: () => api.get('/admin-panel/stats/system'),  
  getCurriculumAnalytics: (curriculumId: number) =>api.get(`/admin-panel/analytics/curriculum/${curriculumId}`),
  getTopPerformers: (limit: number = 10) =>api.get(`/admin-panel/students/top-performers?limit=${limit}`),
  getAllStudentsPerformance: () => api.get('/admin-panel/students/performance'),
  getStudentPerformance: (studentId: number) =>api.get(`/admin-panel/students/${studentId}/performance`),
